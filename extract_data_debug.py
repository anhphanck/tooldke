...
def extract_values_from_image(image_path, debug=False):
    try:
        ...
        if not delta_t_match:
            ...
            for num_str in numbers:
                ...
                if num and 10 < num < 100:
                    values['Delta_t'] = num
                    if debug:
                        print(f"Found Delta_t (fallback): {values['Delta_t']}")
                    break
        else:
            values['Delta_t'] = clean_number(delta_t_match.group(1))
            if debug:
                print(f"Found Delta_t: {values['Delta_t']}")
        ...
        if len(meas_parts) >= 2:
            ...
            if max_numbers:
                ...
                if max_candidates:
                    ...
                    if debug:
                        print(f"Found Maximum candidates: {max_candidates} → selected: {values['Maximum']}")
        ...
        if len(meas_parts) >= 3:
            ...
            if mean_numbers:
                ...
                if mean_candidates:
                    ...
                    if debug:
                        print(f"Found Mean candidates: {mean_candidates} → selected: {values['Mean']}")
        ...
        if len(meas_parts) >= 4:
            ...
            if rms_numbers:
                ...
                if rms_candidates:
                    ...
                    if debug:
                        print(f"Found RMS candidates: {rms_candidates} → selected: {values['RMS']}")
...
def main():
    ...
    output_file = r'D:\dke\code\extracted_data_debug.xlsx'
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.png'):
            ...
            if t_value is not None:
                image_values = extract_values_from_image(file_path, debug=True)
